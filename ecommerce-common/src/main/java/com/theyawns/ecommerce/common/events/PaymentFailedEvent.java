 ...
 *
 * <p><strong>Note:</strong> This is a <em>failure trigger event</em>, not a saga step
 * participant. It does not implement {@code SagaEvent} because it is neither a forward
 * event (needing compensation) nor a compensation event (undoing a forward event).
 * Instead, saga listeners subscribe to this event directly via ITopic and initiate
 * compensation of prior steps. See {@code StockReservationFailedEvent} for the same pattern.
 *
